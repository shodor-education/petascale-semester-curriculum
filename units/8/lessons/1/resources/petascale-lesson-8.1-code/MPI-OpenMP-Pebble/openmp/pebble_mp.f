...
! File: MPI-OpenMP-Pebble/openmp/pebble_mp.f
...
!     put another source point at a later time
       if( it .gt. 50 ) then
        Ucur(ipeb+100,kpeb+50)=Ucur(ipeb+100,kpeb+50)+Pebble(it-50)
        Ucur(ipeb-100,kpeb-50)=Ucur(ipeb-100,kpeb-50)+Pebble(it-50)
       endif
       if( it .gt. 30 ) then
        Ucur(ipeb+120,kpeb+20)=Ucur(ipeb+120,kpeb+20)+Pebble(it-30)
       endif
       if( it .gt. 60 ) then
        Ucur(ipeb-100,kpeb+50)=Ucur(ipeb-100,kpeb+50)+Pebble(it-60)
       endif
!
...
!
     ...
     *         access="direct",recl=Nrecl)
       ...
       nfchar = 25
!
!      Build GraphicsMagik header file and file name file.
!
