 ...
 * Unit 8: OpenACC
 * Lesson 4: N-Body Mechanics in OpenACC
 * File: N-body-OpenACC4.cpp
...
int main() {
   ...
   double *restrict x,*restrict y,*restrict z;                  // For storing x, y, & z positions as a function of time
   double *restrict ax,*restrict ay,*restrict az;               // For ax, ay, & az accelerations as a function of time
   ...
   // Before we do any of the calculations, we need to begin acc region to define what variables need to 
   // be copied in, out, and are local


#pragma acc data copy(x[0:Ntime*Nbodies],y[0:Ntime*Nbodies],z[0:Ntime*Nbodies]) create(ax[0:Nbodies],ay[0:Nbodies],az[0:Nbodies],vx,vy,itimep1,itime,itimem1,dx,dy,dz,r,F)
   {
...
#pragma acc parallel loop private(dx,dy,dz,r,F)
   for(int ibody1=0;ibody1<Nbodies;ibody1++) { 
	   dx=x[index(itime,ibody1,Nbodies)];
           ...
           for (int ibody2=ibody1+1;ibody2<Nbodies;ibody2++) {
                   dx=x[index(itime,ibody1,Nbodies)]-x[index(itime,ibody2,Nbodies)];
                   dy=y[index(itime,ibody1,Nbodies)]-y[index(itime,ibody2,Nbodies)];
                   dz=z[index(itime,ibody1,Nbodies)]-z[index(itime,ibody2,Nbodies)];
                   r=sqrt(dx*dx+dy*dy+dz*dz)+1.0e7; // 1.0e7 is to soften the potential, prevent "explosions"
