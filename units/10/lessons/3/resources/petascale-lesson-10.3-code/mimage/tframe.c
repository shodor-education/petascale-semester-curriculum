...
//
//
//   tframe makes the test images from the Vis lesson....
//
...
//    
  ...
  fnameptr  =   fopen ("../testGraphicsMagick/data/gray_frame.filenames","r");

  fhdrptr  =   fopen ("../testGraphicsMagick/data/gray_frame.hdr","r");
...
// sprintf(dirfilename,"../testImageMagik/data/gray_frame00000.dat");
   sprintf(dirfilename,"../testGraphicsImageMagick/data/gray_frame00000.dat");
...
//  open frame file
  ...
  fgets(filename,25,fnameptr);
// add the directory to head of data file name
  for (int icm=0;icm<25;icm++)
    {
       dirfilename[icm+27] = filename[icm];
    }
