...
/*
 * Kernel - Parallel reduce sum using global memory
 * ----------------------------
...
/*
 * Kernel - Parallel reduce sum with shared memory
 * ----------------------------
 *   Find the sum of the vector elements in parallel
 *
 *   in: vector with input values
 ...
 */
 __global__ void parallel_reduce_shared(int *in, int *out) {
	...
	for(int i=blockDim.x/2;i>0;i >>= 1){
		if (t_id < i)	
			shared_data[t_id] +=shared_data[t_id + i]; //add the corresponding element from other half 
		//we need to synchronize so that all the threads complete the first operations
	    __syncthreads();
	}
