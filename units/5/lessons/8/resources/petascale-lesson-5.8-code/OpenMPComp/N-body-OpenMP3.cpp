...
int main() {
   ...
   int Nbodies=100000;                   // Number of Bodies
   ...
   itime=0;
   // In order to parallelize we have to change rand to rand_r
...
#pragma omp parallel private(tempx,tempy,tempz,tempr,prob1,prob2)   
   {
...
#pragma omp for   
   ...
   // We will set the Kinetic energy of each particle to be KEfrac*abs(pot[ibody]) 
...
void acceleration(double x[], double y[], double z[], int itime, int Nbodies, double mass, double ax[], double ay[], double az[]) {
	...
	double dx, dy, dz, r; // temporary variables to hold the components of the displacement vector and
